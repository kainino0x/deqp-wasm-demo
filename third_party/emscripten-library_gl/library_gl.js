...
var LibraryGL = {
  ...
  $GL: {
    ...
    setErrorForObject: function(objectHandleArray, objectID, errorIfNull, errorIfUndef, errorIfZero) {
      if (objectID == 0) {
        if (errorIfZero) {
          GL.recordError(errorIfZero);
          return true;
        }
        return false;
      }
      if (objectHandleArray[objectID] === null) {
        GL.recordError(errorIfNull);
        return true;
      }
      if (!objectHandleArray[objectID]) {
        GL.recordError(errorIfUndef);
        return true;
      }
      return false;
    },
        ...
        case "object":
          if (result === null) {
            ...
            for (var i = 0; i < result.length; ++i) {
              switch (type) {
                case 'Integer64': {{{ makeSetValue('p', 'i*4', 'result[i]',     'i64') }}};   break;
...
#endif
    GLctx['compressedTexImage2D'](target, level, internalFormat, width, height, border, data ? {{{ makeHEAPView('U8', 'data', 'data+imageSize') }}} : new Uint8Array(0));
  },
  ...
  glTexImage2D: function(target, level, internalFormat, width, height, border, format, type, pixels) {
    ...
    if (GL.currentContext.supportsWebGL2EntryPoints) {
      ...
      if (GLctx.currentPixelUnpackBufferBinding) {
        ...
        var heapForType = emscriptenWebGLGetHeapForType(type);
        if (!heapForType) {
          GL.recordError(0x0500 /* GL_INVALID_ENUM */);
          return;
        }
        GLctx.texImage2D(target, level, internalFormat, width, height, border, format, type, heapForType, pixels >> emscriptenWebGLGetShiftForType(type));
      } else {
  ...
  glTexSubImage2D: function(target, level, xoffset, yoffset, width, height, format, type, pixels) {
    ...
    if (GL.currentContext.supportsWebGL2EntryPoints) {
      ...
      if (GLctx.currentPixelUnpackBufferBinding) {
        ...
        GLctx.texSubImage2D(target, level, xoffset, yoffset, width, height, format, type, heapForType, pixels >> emscriptenWebGLGetShiftForType(type));
      } else {
  ...
  glReadPixels: function(x, y, width, height, format, type, pixels) {
    ...
    if (GL.currentContext.supportsWebGL2EntryPoints) { // WebGL 2 provides new garbage-free entry points to call to WebGL. Use those always when possible.
      if (GLctx.currentPixelPackBufferBinding) {
        ...
        GLctx.readPixels(x, y, width, height, format, type, heapForType, pixels >> emscriptenWebGLGetShiftForType(type));
      }
      return;
    }
#endif
...
#endif
    if (GL.setErrorForObject(GL.timerQueriesEXT, id, 0x0502, 0x0502)) { return; }
    var query = GL.timerQueriesEXT[id];
  ...
  glTexImage3D: function(target, level, internalFormat, width, height, depth, border, format, type, pixels) {
    if (GLctx.currentPixelUnpackBufferBinding) {
      ...
      GLctx['texImage3D'](target, level, internalFormat, width, height, depth, border, format, type, heapForType, pixels >> emscriptenWebGLGetShiftForType(type));
    } else {
  ...
  glTexSubImage3D: function(target, level, xoffset, yoffset, zoffset, width, height, depth, format, type, pixels) {
    if (GLctx.currentPixelUnpackBufferBinding) {
      ...
      GLctx['texSubImage3D'](target, level, xoffset, yoffset, zoffset, width, height, depth, format, type, heapForType, pixels >> emscriptenWebGLGetShiftForType(type));
    } else {
...
#endif
    if (GL.setErrorForObject(GL.queries, id, 0x0502, 0x0502, 0x0501)) { return; }
  ...
  glSamplerParameterf: function(sampler, pname, param) {
    ...
    GLctx['samplerParameterf'](sampler, pname, param);
  },
  ...
  glSamplerParameteri: function(sampler, pname, param) {
    ...
    GLctx['samplerParameteri'](sampler, pname, param);
  },
...
#endif
    if (GL.setErrorForObject(GL.programs, program, 0x0501, 0x0502, 0x0501)) { return; }
  ...
  glGetUniformIndices: function(program, uniformCount, uniformNames, uniformIndices) {
    ...
    if (GL.setErrorForObject(GL.programs, program, 0x0501, 0x0502)) { return; }
    program = GL.programs[program];
    var names = [];
  ...
  glClientWaitSync: function(sync, flags, timeoutLo, timeoutHi) {
    ...
    if (GL.setErrorForObject(GL.syncs, sync, 0x0502, 0x0502, 0x0501)) { return; }
...
#endif
    if (GL.setErrorForObject(GL.programs, program, 0x0501, 0x0502)) { return; }
    if (GL.setErrorForObject(GL.uniforms, location, 0x0502, 0x0502)) { return; }
...
#if FULL_ES2
    if (index >= GL.currentContext.clientBuffers.length) {
      GL.recordError(0x0501 /* GL_INVALID_VALUE */);
      return;
    }
...
#endif
    if (GL.setErrorForObject(GL.shaders, shader, 0x0501, 0x0502)) { return; }
